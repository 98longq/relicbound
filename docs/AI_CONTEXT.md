...
- 默认分支：`main`
- 当前阶段：M1 角色基础控制原型
- 当前优先级：用户本地运行验证 M1；验证通过后进入 M2 基础战斗闭环
...
如果用户反馈 M1 能正常运行，则优先进入 M2：基础战斗闭环。
...
如果用户反馈 M1 不能运行，则优先修复运行问题，不进入 M2。
