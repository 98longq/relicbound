...
- 默认分支：`main`
- 当前阶段：M2 基础战斗闭环
- 当前优先级：用户本地运行验证 M2；验证通过后进入 M3 敌人 AI 与掉落系统
...
2. M1：玩家基础控制原型。已提交。
3. M2：基础战斗闭环。最小战斗原型已提交，等待用户本地运行验证。
4. M3：敌人 AI 与掉落系统。
5. M4：装备与背包原型。
6. M5：Boss 与 Demo 闭环。
7. M6：遗物系统与 Build 雏形。
...
如果用户反馈 M2 能正常运行，则优先进入 M3：敌人 AI 与掉落系统。
...
如果用户反馈 M2 不能运行，则优先修复运行问题，不进入 M3。
